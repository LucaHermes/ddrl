...
class LegCoupling(tf.keras.layers.Layer):

    def build(self, input_shape):
        self.n_dims = input_shape[-1]//2
        #self.coupling_mean = tf.Variable(np.zeros((4, self.n_dims)),
        #        dtype=tf.float32, name='leg_coupling_mean')
        #self.coupling_std = tf.Variable(np.ones((4, self.n_dims)),
        #        dtype=tf.float32, name='leg_coupling_std')
        #self.coupling = tf.Variable([[-1, -1],[0.7, 0.7],[-0.7, -0.7],[1, 1]],
        self.coupling = tf.Variable([[1, 1],[-1, -1],[-1, -1],[1, 1]],
                dtype=tf.float32, name='leg_coupling')

    def call(self, x, node_id_input):
        #coupling = tf.random.normal(self.coupling_mean.shape)
        #coupling = coupling * self.coupling_std + self.coupling_mean
        #coupling = tf.pad(coupling, [[0,0],[0,self.n_dims]], constant_values=1)
        #coupling_coeff = tf.gather(coupling, node_id_input[...,0])
        coupling = tf.pad(self.coupling, [[0,0],[0,self.n_dims]], constant_values=1)
        coupling_coeff = tf.gather(coupling, node_id_input[...,0])
        return x * coupling_coeff

class FullyConnectedNetwork_Coupling_GlorotUniformInitializer(TFModelV2):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name):
        super(FullyConnectedNetwork_Coupling_GlorotUniformInitializer, self).__init__(
        ...
        # apply leg coupling
        logits_out = self.leg_coupling(logits_out, node_id_input)
        ...
        self.register_variables(self.leg_coupling.variables)
